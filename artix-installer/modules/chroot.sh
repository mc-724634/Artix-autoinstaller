...
phase9_chroot_config() {
    ...
    _write_post_script "$timezone" "$hostname" "${FIRMWARE:-bios}" "${INIT:-dinit}"
...
# 9.4 packages
pacman -Sy --noconfirm grub chrony
EOF

    _append_network_setup "$init"
...
# _append_network_setup INIT — appends iwd + NetworkManager install and
# first-boot service enabling to /mnt/root/post.sh, using the package
# names and enable mechanism appropriate to the chosen init system.
# NetworkManager is also pointed at iwd as its Wi-Fi backend, since
# wpa_supplicant is never installed in this setup.
_append_network_setup() {
    local init="$1" pkgs enable_lines

    case "$init" in
        dinit)
            pkgs="iwd iwd-dinit networkmanager networkmanager-dinit"
            enable_lines='mkdir -p /etc/dinit.d/boot.d
ln -sf /etc/dinit.d/iwd /etc/dinit.d/boot.d/iwd
ln -sf /etc/dinit.d/networkmanager /etc/dinit.d/boot.d/networkmanager'
            ;;
        openrc)
            pkgs="iwd iwd-openrc networkmanager networkmanager-openrc"
            enable_lines='rc-update add iwd default
rc-update add NetworkManager default'
            ;;
        runit)
            pkgs="iwd iwd-runit networkmanager networkmanager-runit"
            enable_lines='ln -sf /etc/runit/sv/iwd /etc/runit/runsvdir/default/
ln -sf /etc/runit/sv/NetworkManager /etc/runit/runsvdir/default/'
            ;;
        s6)
            pkgs="iwd iwd-s6 networkmanager networkmanager-s6"
            # NOTE: Artix's s6 service-enabling mechanism (s6-rc / the 66
            # suite, depending on variant) is less consistently documented
            # than the other inits. This uses the s6-rc-bundle approach;
            # verify against the live current Artix wiki for your s6
            # variant if the services don't come up after reboot.
            enable_lines='s6-rc-bundle-update -c /etc/s6/rc.d -l /etc/s6/rc.d/compiled add default iwd networkmanager 2>>/root/post-install.log || true'
            ;;
        systemd)
            pkgs="iwd networkmanager"
            enable_lines='systemctl enable iwd.service
...
# Use iwd as NetworkManager's Wi-Fi backend (no wpa_supplicant installed).
...
# Enable iwd + NetworkManager for first boot.
${enable_lines}
EOF
}
...
phase10_user_setup() {
    local username password password2 grant_sudo
    ...
    ui_yesno "Grant $username sudo access (admin privileges)?" 7 55
    grant_sudo=$?  # 0 = yes, 1 = no/cancel

    if [[ $grant_sudo -eq 0 ]]; then
    ...
    if [[ $grant_sudo -eq 0 ]]; then
        ...
        conf_set "USER_SUDO" "yes"
    else
        conf_set "USER_SUDO" "no"
    fi
}
...
phase11_cleanup() {
    ...
    if [[ -f /etc/pacman.conf.artix-installer.bak ]]; then
        mv /etc/pacman.conf.artix-installer.bak /etc/pacman.conf
        rm -f /etc/pacman.d/mirrorlist-archlinux
    fi
